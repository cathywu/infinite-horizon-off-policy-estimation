...
def train_density_ratio(SASR, policy0, policy1, den_discrete, gamma):
    for sasr in SASR:
        ...
        for state, action, next_state, reward in sasr:
            ...
            den_discrete.feed_data(state, next_state, initial_state,
                                   policy_ratio, discounted_t)
        den_discrete.feed_data(-1, initial_state, initial_state, 1,
                               1 - discounted_t)
...
def off_policy_evaluation_density_ratio(SASR, policy0, policy1, density_ratio,
    ...
    for sasr in SASR:
        ...
        for state, action, next_state, reward in sasr:
            policy_ratio = policy1[state, action] / policy0[state, action]
            total_reward += density_ratio[
                                state] * policy_ratio * reward * discounted_t
            self_normalizer += density_ratio[
                                   state] * policy_ratio * discounted_t
            discounted_t *= gamma
...
def importance_sampling_estimator(SASR, policy0, policy1, gamma):
    ...
    for sasr in SASR:
        ...
        self_normalizer = 0.0
        for state, action, next_state, reward in sasr:
            log_trajectory_ratio += np.log(policy1[state, action]) - np.log(
                policy0[state, action])
            total_reward += reward * discounted_t
            self_normalizer += discounted_t
            discounted_t *= gamma
...
def importance_sampling_estimator_stepwise(SASR, policy0, policy1, gamma):
    ...
    for sasr in SASR:
        ...
        self_normalizer = 0.0
        for state, action, next_state, reward in sasr:
            step_log_pr += np.log(policy1[state, action]) - np.log(
                policy0[state, action])
...
def weighted_importance_sampling_estimator_stepwise(SASR, policy0, policy1,
    ...
    for sasr in SASR:
        ...
        self_normalizer = 0.0
        for state, action, next_state, reward in sasr:
            log_pr = np.log(policy1[state, action]) - np.log(
                policy0[state, action])
            if log_policy_ratio:
    ...
    for i in range(REW.shape[0]):
        est_reward += np.sum(
            rho[i] / np.mean(rho, axis=0) * REW[i]) / self_normalizer
...
def Q_learning(env, num_trajectory, truncate_size, temperature=2.0):
    ...
    for k in range(20):
        ...
        SAS, f, avr_reward = roll_out(n_state, env, pi, num_trajectory,
                                      truncate_size)
    ...
    for state in range(n_state):
        for next_state in range(n_state):
            for action in range(n_action):
                Tpi[state, next_state] += T[state, action, next_state] * pi[
                    state, action]
    dt = d0 / np.sum(d0)
...
if __name__ == '__main__':
    estimator_name = ['On Policy', 'Density Ratio', 'Naive Average', 'IST',
                      'ISS', 'WIST', 'WISS', 'Model Based']
    length = 5
    env = taxi(length)
    n_state = env.n_state
    ...
    for k in range(20):
        ...
        res[1:, k] = run_experiment(n_state, n_action, np.array(SASR0),
                                    pi_behavior, pi_target, gm)
        ...
        print('------seed = {}------'.format(k))
        for i in range(8):
            print('  ESTIMATOR: ' + estimator_name[i] + ', rewards = {}'.format(
                res[i, k]))
        print('----------------------')
        sys.stdout.flush()
