...
class toy_Markov(object):
    ...
    def initial_state(self):
        self.cur_state = np.random.choice(self.number_state,
...
class taxi(object):
    ...
    def __init__(self, length):
        ...
        self.x = np.random.randint(length)
        self.y = np.random.randint(length)
        self.possible_passenger_loc = [(0, 0), (0, length - 1), (length - 1, 0),
                                       (length - 1, length - 1)]
    ...
    def state_encoding(self):
        length = self.length
        return self.taxi_status + (self.passenger_status + (
        self.x * length + self.y) * 16) * 5
